	...
	<div class="transfer flex">
		<div class="transfer-left">
			...
			<el-pagination
				...
				:page-size="pageSize"
				:total="keyWord ? filterList.length : checkAllList.length"
			></el-pagination>
		</div>
...
module.exports = {
	...
	watch: {
		sourcelist: {
			handler(newVal) {
				...
				this.checkAllList = newVal.filter(
					item =>
						!this.selectedList.some(
							items => items[parent] == item[parent] && items[id] === item[id]
						)
				);
	...
	methods: {
		...
		handleCheckAll(val) {
			this.checkedValue = val ? this.currentData.map(item => item.id) : [];
		...
		handleSelectedAll(val) {
			this.selectedValue = val ? this.selectedList.map(item => item.id) : [];
